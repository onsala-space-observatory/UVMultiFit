...
double *QuinnFringe::getRates(){
    return &Rates[0];
};

double *QuinnFringe::getDelays(){
    return &Delays[0];
};

double *QuinnFringe::getPhases(){
    return &Phases[0];
};

double *QuinnFringe::getBins(){
...
QuinnFringe::QuinnFringe(int Na, int Nt, int Nc, cplx64f *ObsV, cplx64f *ModV,
                         int *A1, int *A2, double *tA, char *dofit, double *freqs, double *wgts)
{
    ObsVis = ObsV;
    ModVis = ModV;
    Ant1 = A1;
    ...
    //  printf("Freqs: %i  %.3e %.3e  %.3e\n",Nchan, Freqs[0],Freqs[Nchan],Dnu);
...
int QuinnFringe::GFF(int REFANT, int DOGLOBAL, int DOMODEL)
{
    ...
    int i0, i1;
    switch (DOGLOBAL) {
      case 0: i0 = REFANT; i1 = REFANT+1; break;
      case 1: i0 = 0; i1 = Nant; break;
      default:
        printf("\n UNRECOGNIZED GLOBAL OPTION. WILL NOT GLOBALIZE!");
        i0 = REFANT; i1 = REFANT+1;
    };
    ...
    //int TotDim = Nchan*NData[0];
    int MaxDim = Nchan*NData[0];
    double BasWgt;
    int prevNvis = NData[0];
    double tmin,tmax;

    for (j=1; j<NBas; j++){if(NData[j]*Nchan>MaxDim){MaxDim=NData[j]*Nchan;};};
        ...
        if (NData[j] != 0){
            ...
            for (k=0; k<Ntime; k++){
                ...
                if (BNum==j && fittable[k]){
                    a1 = a1sel; a2 = a2sel;
                    inu = NcurrVis*Nchan; knu = k*Nchan;
                    if (DOMODEL == 0){
                    ...
                    Tindex[j][NcurrVis] = k;
                    NcurrVis += 1;
                    if(Times[k]>tmax){tmax=Times[k];};
                    if(Times[k]<tmin){tmin=Times[k];};
                    //      printf("BW: %.3e\n",BasWgt);
                };
            };
            BasWgt /= ((double) Nchan);
            Dtime = tmax-tmin;
            if (Dtime>0.0 && DtMin>Dtime){DtMin = Dtime;};
            ...
            if (NcurrVis > 1){
                ...
                if (Nchan>npix){Chi = npix/2; Chf = Nchan-npix/2;}
                else  {Chi = Nchan/2; Chf = Nchan/2;};

                if (NcurrVis>npix){t0 = npix/2; tf = NcurrVis-npix/2;}
                else  {t0 = NcurrVis/2; tf = NcurrVis/2;};
                ...
                // Unwrap:
                if (nu[1]==0){nu[0]=Nchan-1;} else{nu[0]=nu[1]-1;};
                if (nu[1]==Nchan-1){nu[2]=0;} else{nu[2]=nu[1]+1;};
                if (ti[1]==0){ti[0]=NcurrVis-1;} else{ti[0]=ti[1]-1;};
                if (ti[1]==NcurrVis-1){ti[2]=0;} else{ti[2]=ti[1]+1;};
                ...
                Weights[BasNum[a1][a2]] = BasWgt;

                //  printf("a1: %i  a2: %i , Wgt: %.3e, Del: %.3e Rat: %.3e \n",
                // a1,a2,Weights[BasNum[a1][a2]],BLDelays[BasNum[a1][a2]],BLRates[BasNum[a1][a2]]);

            } else {   // Comes from if(NcurrVis > 1)
                BLRates[BasNum[a1][a2]] = 0.0; BLRates[BasNum[a1][a2]] = 0.0;
                BLDelays[BasNum[a1][a2]] = 0.0; BLDelays[BasNum[a1][a2]] = 0.0;
    ...
    for (i=0; i<Nant; i++){
        for(j=0;j<Nant;j++){
            if((NData[BasNum[i][j]]>0 || NData[BasNum[j][i]]>0)&& i!=j && i!=REFANT
               && (Weights[BasNum[i][j]]>0. ||  Weights[BasNum[j][i]]>0.)) {
                calAnt[NcalAnt] = i;
                NcalAnt ++; break;
                //  printf("%i %i %i\n",i,j,NcalAnt); break;
            };
        };
        //  if(i!= REFANT && TotData){NcalAnt ++;};
    };
    ...
    /* k=0;
       ...
       bool TotData = false;
       for(j=0;j<Nant;j++){
       if((NData[BasNum[i][j]]>0|| NData[BasNum[i][j]]>0) && i!=j&& i!=REFANT){calAnt[k]=i;k++; break;};
       };
       //  if(i!= REFANT && TotData){calAnt[k] = i; k++;};
       };
    */
    ...
    /*printf("i0: %i , i1: %i \n",i0,i1);
      for(i=0;i<NcalAnt;i++){printf("i: %i , calAnt: %i \n",i,calAnt[i]);};
      for(i=0;i<NBas;i++){
      for (j=0;j<Nant;j++){
      for (k=0;k<Nant;k++){
      if (BasNum[j][k]==i){printf("Bas: %i / %i-%i, NData: %i \n",i,j,k,NData[i]);};
      };
      };
      };
    */
    ...
    */
    ...
    for (i=0;i<Nant;i++){Phases[i] = 0.0;Rates[i] = 0.0;Delays[i] = 0.0;};
    ...
    fftw_free(BufferVis); fftw_free(out);
    return 0;
};

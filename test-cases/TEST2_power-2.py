...
sim = Simulation(array_config="alma.out10.cfg", center_freq=50.0, pixels=1000, cell_size=0.01,
                 freq_channels=100, channel_width=0.01, image_name="power-2", total_time=300)
...
if not Path(skymodel).exists():
    if Path("Disc/Disc.alma.out10.noisy.ms").exists():
        ...
        for i in range(sim.freq_channels):
            ...
            data[:, :, 0, i] = 1/(RSq + source_radius**2.0)
...
if not Path(vis).exists():
    ...
    LAS = 1.0/(image_size/3600.0*np.pi/180.0)*3.0e8/(sim.center_freq*1.0e9)
    config = os.path.splitext(sim.array_config)[0]
    flagdata(f"{sim.image_name}/{sim.image_name}.{config}.ms",
             mode="manual", uvrange=f"<{LAS:.1f}m")

    flagdata(f"{sim.image_name}/{sim.image_name}.{config}.noisy.ms",
             mode="manual", uvrange=f"<{LAS:.1f}m")

model = [sim.image_name]
modvars = "p[2],p[3],p[0],p[1],1.0,0.0"
modbounds = [[0.0, None], [0.0, diameter*1.5], [-1.0, 1.0], [-1.0, 1.0]]

size = float(si["major"].split('a')[0])
minor = float(si["minor"].split('a')[0])
initial = [0.8, diameter*1.2, 0.0, 0.0]
